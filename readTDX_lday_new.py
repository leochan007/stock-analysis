...
def classify_market(code):
    if code.startswith('60') or code.startswith('68'):
        ...
        return 'sz'
    elif code.startswith('83') or code.startswith('87'):  # 北交所
        return 'bj'
    else:
        return 'unknown'
...
def update_lday():
    ...
    tdx_stocks['market'] = tdx_stocks[0].apply(classify_market)
    
    grouped = tdx_stocks.groupby('market')[0].apply(list).to_dict()
    
    for market, codes in grouped.items():
        if market == 'unknown':
            print(codes)
            continue
        print("从通达信%s导出数据" % market)
        for code in tqdm(codes):
            f = f"{market}{code}.day"
            if os.path.exists(f"{ucfg.tdx['tdx_path']}/vipdoc/{market}/lday/{f}"):
                func.day2csv(f"{ucfg.tdx['tdx_path']}/vipdoc/{market}/lday", f, ucfg.tdx['csv_lday'])
    ...
    for i in tqdm(ucfg.index_list):
        ...
        if 'sh' in i:
            ...
            func.day2csv(ucfg.tdx['tdx_path'] + '/vipdoc/bj/lday', i, ucfg.tdx['csv_index'])
...
def qfq(file_list, df_gbbq, cw_dict, tqdm_position=None):
    ...
    for filename in tq:
        ...
        if len(df_qfq) > 0:  # 返回值大于0，表示有更新
            df_qfq.to_csv(ucfg.tdx['csv_lday_qfq'] + os.sep + filename, index=False, encoding='gbk')
